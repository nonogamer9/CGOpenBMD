	...
	internal class NewLocation
	{
		...
		private Point FindNewLocationWithoutMaxAngle(Vertex torg, Vertex tdest, Vertex tapex, ref double xi, ref double eta, bool offcenter, Otri badotri)
		{
			...
			if (num7 == 1)
			{
				...
				bool flag = num28 < 0.0 || ((Math.Abs(num28 - 0.0) <= 1E-50) ? true : false);
				...
				else
				{
					...
					if (!neighborsVertex2)
					{
						...
						double num44;
						double num45;
						if (ChooseCorrectPoint(x3, y3, p[3], p[4], point.x, point.y, false))
						{
							num44 = p[3];
							num45 = p[4];
						}
		...
		private Point FindNewLocation(Vertex torg, Vertex tdest, Vertex tapex, ref double xi, ref double eta, bool offcenter, Otri badotri)
		{
			...
			if (num7 == 1)
			{
				...
				bool flag = num30 < 0.0 || ((Math.Abs(num30 - 0.0) <= 1E-50) ? true : false);
				num11 = DoSmoothing(deltri, torg, tdest, tapex, ref newloc);
				if (num11 > 0)
				{
					Statistic.RelocationCount++;
				...
				else
				{
					...
					double num59;
					double num60;
					if (ChooseCorrectPoint(num55, num56, point3.x, point3.y, num53, num54, true))
					{
						num57 = num53;
						num58 = num54;
						num59 = num55;
					...
					if (!neighborsVertex2)
					{
						...
						double num65;
						double num66;
						if (ChooseCorrectPoint(num72, num73, p[3], p[4], point.x, point.y, false))
						{
							num65 = p[3];
							num66 = p[4];
						}
		...
		private double MinDistanceToNeighbor(double newlocX, double newlocY, ref Otri searchtri)
		{
			...
			else
			{
				...
				if (num < 0.0)
				{
					searchtri.SymSelf();
					searchtri.Copy(ref o);
					locateResult = mesh.locator.PreciseLocate(point, ref o, false);
				}
				else if (num == 0.0)
				{
