	...
	public static class pb_HandleUtility
	{
		...
		public static bool EdgeRaycast(Camera cam, Vector2 mousePosition, pb_Object mesh, pb_Edge[] edges, Vector3[] verticesInWorldSpace, out pb_Edge edge)
		{
			...
			else
			{
				...
				if (FaceRaycast(inWorldRay, mesh, out hits, float.PositiveInfinity, Culling.FrontBack, (HashSet<pb_Face>)null))
				{
					...
					for (int num3 = 0; num3 < hits.Count; num3++)
					{
						...
						for (int num4 = 0; num4 < allEdges.Length; num4++)
						{
							pb_Edge pb_Edge2 = allEdges[num4];
