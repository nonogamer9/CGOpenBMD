...
namespace ProBuilder2.Common
{
	public class pb_ShapeGenerator
	{
		...
		public static pb_Object CylinderGenerator(int axisDivisions, float radius, float height, int heightCuts, int smoothing = -1)
		{
			...
			for (int l = 0; l < heightCuts + 1; l++)
			{
				for (int m = 0; m < axisDivisions * 4; m += 4)
				{
					...
					int num8 = num5 + 2;
					int num9 = num5 + 3;
					array3[num4++] = new pb_Face(new int[6] { num6, num7, num8, num7, num9, num8 }, pb_Constant.DefaultMaterial, new pb_UV(), smoothing, -1, -1, false);
				}
			}
		...
		public static pb_Object PlaneGenerator(float _width, float _height, int widthCuts, int heightCuts, Axis axis, bool smooth)
		{
			...
			Vector3[] array4 = pb_Object2.VerticesInWorldSpace();
			...
			Vector3 offset = pb_Object2.transform.position - zero;
			pb_Object2.transform.position = zero;
			pb_Object2.TranslateVertices_World(pb_Object2.msh.triangles, offset);
			return pb_Object2;
		}
		...
		public static pb_Object IcosahedronGenerator(float radius, int subdivisions)
		{
			...
			{
				array[i] = ICOSAHEDRON_VERTICES[ICOSAHEDRON_TRIANGLES[i]].normalized * radius;
				array[i + 1] = ICOSAHEDRON_VERTICES[ICOSAHEDRON_TRIANGLES[i + 1]].normalized * radius;
				array[i + 2] = ICOSAHEDRON_VERTICES[ICOSAHEDRON_TRIANGLES[i + 2]].normalized * radius;
			}
