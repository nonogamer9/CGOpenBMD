...
namespace TMPro
{
	...
	public class TextMeshProUGUI : TMP_Text, ILayoutElement
	{
		...
		protected override void GenerateTextMesh()
		{
			...
			for (int i = 0; i < m_char_buffer.Length && m_char_buffer[i] != 0; i++)
			{
				...
				if (m_textElementType == TMP_TextElementType.Character && !isUsingAlternateTypeface && ((m_style & FontStyles.Bold) == FontStyles.Bold || (m_fontStyle & FontStyles.Bold) == FontStyles.Bold))
				{
					if (m_currentMaterial.HasProperty(ShaderUtilities.ID_GradientScale))
					{
						...
						num6 = m_currentFontAsset.boldStyle / 4f * num22 * m_currentMaterial.GetFloat(ShaderUtilities.ID_ScaleRatio_A);
						if (num6 + num5 > num22)
						{
							num5 = num22 - num6;
						}
				...
				else
				{
					if (m_currentMaterial.HasProperty(ShaderUtilities.ID_GradientScale))
					{
						float num23 = m_currentMaterial.GetFloat(ShaderUtilities.ID_GradientScale);
						num6 = m_currentFontAsset.normalStyle / 4f * num23 * m_currentMaterial.GetFloat(ShaderUtilities.ID_ScaleRatio_A);
						if (num6 + num5 > num23)
						{
							num5 = num23 - num6;
						}
				...
				float num24 = m_currentFontAsset.fontInfo.Baseline * m_fontScale * m_fontScaleMultiplier * m_currentFontAsset.fontInfo.Scale;
				vector3.x = m_xAdvance + (m_cached_TextElement.xOffset - num5 - num6 + glyphValueRecord.xPlacement) * num2 * (1f - m_charWidthAdjDelta);
				vector3.y = num24 + (m_cached_TextElement.yOffset + num5 + glyphValueRecord.yPlacement) * num2 - m_lineOffset + m_baselineOffset;
				vector3.z = 0f;
				vector4.x = vector3.x;
				...
				{
					float num25 = (float)(int)m_currentFontAsset.italicStyle * 0.01f;
					Vector3 vector7 = new Vector3(num25 * ((m_cached_TextElement.yOffset + num5 + num6) * num2), 0f, 0f);
					Vector3 vector8 = new Vector3(num25 * ((m_cached_TextElement.yOffset - m_cached_TextElement.height - num5 - num6) * num2), 0f, 0f);
					vector3 += vector7;
					vector4 += vector8;
					vector5 += vector7;
					vector6 += vector8;
				...
				m_textInfo.characterInfo[m_characterCount].baseLine = num24 - m_lineOffset + m_baselineOffset;
				m_textInfo.characterInfo[m_characterCount].aspectRatio = (vector5.x - vector4.x) / (vector3.y - vector4.y);
				float num26 = m_currentFontAsset.fontInfo.Ascender * ((m_textElementType != TMP_TextElementType.Character) ? m_textInfo.characterInfo[m_characterCount].scale : (num2 / num18)) + m_baselineOffset;
				...
				float num27 = m_currentFontAsset.fontInfo.Descender * ((m_textElementType != TMP_TextElementType.Character) ? m_textInfo.characterInfo[m_characterCount].scale : (num2 / num18)) + m_baselineOffset;
				float num28 = (m_textInfo.characterInfo[m_characterCount].descender = num27 - m_lineOffset);
				m_maxLineDescender = ((!(num27 < m_maxLineDescender)) ? m_maxLineDescender : num27);
				if ((m_style & FontStyles.Subscript) == FontStyles.Subscript || (m_style & FontStyles.Superscript) == FontStyles.Superscript)
				{
					...
					m_maxLineAscender = ((!(num29 > m_maxLineAscender)) ? m_maxLineAscender : num29);
					float num30 = (num27 - m_baselineOffset) / m_currentFontAsset.fontInfo.SubSize;
					num27 = m_maxLineDescender;
				...
				if (num4 == 9 || num4 == 160 || num4 == 8199 || (!char.IsWhiteSpace((char)num4) && num4 != 8203) || m_textElementType == TMP_TextElementType.Sprite)
				{
					...
					bool flag9 = (m_lineJustification & (TextAlignmentOptions)0x10) == (TextAlignmentOptions)16 || (m_lineJustification & (TextAlignmentOptions)8) == (TextAlignmentOptions)8;
					...
					if (num16 > num13 * ((!flag9) ? 1f : 1.05f))
					{
						...
						if (base.enableWordWrapping && m_characterCount != m_firstCharacterOfLine)
						{
							...
							{
								float num34 = m_textInfo.characterInfo[m_characterCount].ascender - m_textInfo.characterInfo[m_characterCount].baseLine;
								num9 = 0f - m_maxLineDescender + num34 + (num8 + m_lineSpacing + m_lineSpacingDelta) * num;
								m_lineOffset += num9;
				...
				if (num4 == 9)
				{
					float num36 = m_currentFontAsset.fontInfo.TabWidth * num2;
					float num37 = Mathf.Ceil(m_xAdvance / num36) * num36;
					m_xAdvance = ((!(num37 > m_xAdvance)) ? (m_xAdvance + num36) : num37);
				}
				...
				else if (!m_isRightToLeft)
				{
					...
					if (m_isFXMatrixSet)
					{
						num38 = m_FXMatrix.m00;
					}
					m_xAdvance += ((m_cached_TextElement.xAdvance * num38 * num7 + m_characterSpacing + m_currentFontAsset.normalSpacingOffset + glyphValueRecord.xAdvance) * num2 + m_cSpacing) * (1f - m_charWidthAdjDelta);
					if (char.IsWhiteSpace((char)num4) || num4 == 8203)
					{
						m_xAdvance += m_wordSpacing * num2;
					}
				...
				if (num4 == 10 || m_characterCount == totalCharacterCount - 1)
				{
					if (m_lineNumber > 0 && !TMP_Math.Approximately(m_maxLineAscender, m_startOfLineAscender) && m_lineHeight == -32767f && !m_isNewPage)
					{
						...
						AdjustLineOffset(m_firstCharacterOfLine, m_characterCount, num39);
						num28 -= num39;
						m_lineOffset += num39;
					}
					m_isNewPage = false;
					float num40 = m_maxLineAscender - m_lineOffset;
					...
					m_textInfo.lineInfo[m_lineNumber].characterCount = m_textInfo.lineInfo[m_lineNumber].lastCharacterIndex - m_textInfo.lineInfo[m_lineNumber].firstCharacterIndex + 1;
					m_textInfo.lineInfo[m_lineNumber].visibleCharacterCount = m_lineVisibleCharacterCount;
					m_textInfo.lineInfo[m_lineNumber].lineExtents.min = new Vector2(m_textInfo.characterInfo[m_firstVisibleCharacterOfLine].bottomLeft.x, num41);
					m_textInfo.lineInfo[m_lineNumber].lineExtents.max = new Vector2(m_textInfo.characterInfo[m_lastVisibleCharacterOfLine].topRight.x, num40);
					...
					m_textInfo.lineInfo[m_lineNumber].baseline = 0f - m_lineOffset;
					m_textInfo.lineInfo[m_lineNumber].ascender = num40;
					m_textInfo.lineInfo[m_lineNumber].descender = num41;
					m_textInfo.lineInfo[m_lineNumber].lineHeight = num40 - num41 + num8 * num;
					...
					if (num4 == 10)
					{
						...
						if (m_lineHeight == -32767f)
						{
							num9 = 0f - m_maxLineDescender + num26 + (num8 + m_lineSpacing + m_paragraphSpacing + m_lineSpacingDelta) * num;
							m_lineOffset += num9;
						}
						else
						{
				...
				if (m_overflowMode == TextOverflowModes.Page && num4 != 13 && num4 != 10)
				{
					...
					m_textInfo.pageInfo[m_pageNumber].descender = ((!(num27 < m_textInfo.pageInfo[m_pageNumber].descender)) ? m_textInfo.pageInfo[m_pageNumber].descender : num27);
			...
			float num46 = (m_previousLossyScaleY = base.transform.lossyScale.y);
			...
			float num49 = 0f;
			float num50 = 0f;
			float num51 = TMP_Text.k_LargePositiveFloat;
			int num52 = 0;
			float num53 = 0f;
			float num54 = 0f;
			float b = 0f;
			...
			for (int j = 0; j < m_characterCount; j++)
			{
				...
				case TextAlignmentOptions.CaplineFlush:
				{
					...
					bool flag13 = (textAlignmentOptions & (TextAlignmentOptions)0x10) == (TextAlignmentOptions)16;
					if ((!char.IsControl(character4) && lineNumber < m_lineNumber) || flag13 || tMP_LineInfo.maxAdvance > tMP_LineInfo.width)
					{
						if (lineNumber != num43 || j == 0 || j == m_firstVisibleCharacter)
						{
						...
						float num55 = (m_isRightToLeft ? (tMP_LineInfo.width + tMP_LineInfo.maxAdvance) : (tMP_LineInfo.width - tMP_LineInfo.maxAdvance));
						int num56 = tMP_LineInfo.visibleCharacterCount - 1 + tMP_LineInfo.controlCharacterCount;
						int num57 = ((!characterInfo[tMP_LineInfo.lastCharacterIndex].isVisible) ? (tMP_LineInfo.spaceCount - 1) : tMP_LineInfo.spaceCount) - tMP_LineInfo.controlCharacterCount;
						if (flag10)
						{
							num57--;
							num56++;
						}
						float num58 = ((num57 <= 0) ? 1f : m_wordWrappingRatios);
						if (num57 < 1)
						{
							num57 = 1;
						}
						if (character3 != '\u00a0' && (character3 == '\t' || char.IsSeparator(character3)))
						{
							if (!m_isRightToLeft)
							{
								vector11 += new Vector3(num55 * (1f - num58) / (float)num57, 0f, 0f);
							}
							else
							{
								vector11 -= new Vector3(num55 * (1f - num58) / (float)num57, 0f, 0f);
						...
						{
							vector11 += new Vector3(num55 * num58 / (float)num56, 0f, 0f);
						}
						else
						{
							vector11 -= new Vector3(num55 * num58 / (float)num56, 0f, 0f);
				...
				if (isVisible)
				{
					...
					case TMP_TextElementType.Character:
					{
						...
						float num59 = m_uvLineOffset * (float)lineNumber % 1f;
						...
						case TextureMappingOptions.Line:
							...
							{
								characterInfo[j].vertex_BL.uv2.x = (characterInfo[j].vertex_BL.position.x - lineExtents.min.x) / (lineExtents.max.x - lineExtents.min.x) + num59;
								characterInfo[j].vertex_TL.uv2.x = (characterInfo[j].vertex_TL.position.x - lineExtents.min.x) / (lineExtents.max.x - lineExtents.min.x) + num59;
								characterInfo[j].vertex_TR.uv2.x = (characterInfo[j].vertex_TR.position.x - lineExtents.min.x) / (lineExtents.max.x - lineExtents.min.x) + num59;
								characterInfo[j].vertex_BR.uv2.x = (characterInfo[j].vertex_BR.position.x - lineExtents.min.x) / (lineExtents.max.x - lineExtents.min.x) + num59;
							}
							else
							{
								characterInfo[j].vertex_BL.uv2.x = (characterInfo[j].vertex_BL.position.x + vector11.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num59;
								characterInfo[j].vertex_TL.uv2.x = (characterInfo[j].vertex_TL.position.x + vector11.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num59;
								characterInfo[j].vertex_TR.uv2.x = (characterInfo[j].vertex_TR.position.x + vector11.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num59;
								characterInfo[j].vertex_BR.uv2.x = (characterInfo[j].vertex_BR.position.x + vector11.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num59;
							}
							break;
						...
						case TextureMappingOptions.MatchAspect:
						{
							...
							case TextureMappingOptions.Character:
								...
								break;
							case TextureMappingOptions.Line:
								characterInfo[j].vertex_BL.uv2.y = (characterInfo[j].vertex_BL.position.y - lineExtents.min.y) / (lineExtents.max.y - lineExtents.min.y) + num59;
								characterInfo[j].vertex_TL.uv2.y = (characterInfo[j].vertex_TL.position.y - lineExtents.min.y) / (lineExtents.max.y - lineExtents.min.y) + num59;
								...
								break;
							case TextureMappingOptions.Paragraph:
								characterInfo[j].vertex_BL.uv2.y = (characterInfo[j].vertex_BL.position.y - m_meshExtents.min.y) / (m_meshExtents.max.y - m_meshExtents.min.y) + num59;
								characterInfo[j].vertex_TL.uv2.y = (characterInfo[j].vertex_TL.position.y - m_meshExtents.min.y) / (m_meshExtents.max.y - m_meshExtents.min.y) + num59;
								characterInfo[j].vertex_TR.uv2.y = characterInfo[j].vertex_BL.uv2.y;
								characterInfo[j].vertex_BR.uv2.y = characterInfo[j].vertex_TL.uv2.y;
								break;
							case TextureMappingOptions.MatchAspect:
							...
							float num60 = (1f - (characterInfo[j].vertex_BL.uv2.y + characterInfo[j].vertex_TL.uv2.y) * characterInfo[j].aspectRatio) / 2f;
							characterInfo[j].vertex_BL.uv2.x = characterInfo[j].vertex_BL.uv2.y * characterInfo[j].aspectRatio + num60 + num59;
							characterInfo[j].vertex_TL.uv2.x = characterInfo[j].vertex_BL.uv2.x;
							characterInfo[j].vertex_TR.uv2.x = characterInfo[j].vertex_TL.uv2.y * characterInfo[j].aspectRatio + num60 + num59;
						...
						case TextureMappingOptions.MatchAspect:
						{
							float num61 = (1f - (characterInfo[j].vertex_BL.uv2.x + characterInfo[j].vertex_TR.uv2.x) / characterInfo[j].aspectRatio) / 2f;
							characterInfo[j].vertex_BL.uv2.y = num61 + characterInfo[j].vertex_BL.uv2.x / characterInfo[j].aspectRatio;
							characterInfo[j].vertex_TL.uv2.y = num61 + characterInfo[j].vertex_TR.uv2.x / characterInfo[j].aspectRatio;
							characterInfo[j].vertex_BR.uv2.y = characterInfo[j].vertex_BL.uv2.y;
							characterInfo[j].vertex_TR.uv2.y = characterInfo[j].vertex_TL.uv2.y;
							break;
						}
						}
						num47 = characterInfo[j].scale * (1f - m_charWidthAdjDelta);
						...
						case RenderMode.ScreenSpaceOverlay:
							num47 *= num46 / scaleFactor;
							break;
						case RenderMode.ScreenSpaceCamera:
							num47 *= ((!flag12) ? 1f : num46);
							break;
						...
						characterInfo[j].vertex_BL.uv2.y = num47;
						characterInfo[j].vertex_TL.uv2.x = PackUV(x, y2);
						characterInfo[j].vertex_TL.uv2.y = num47;
						characterInfo[j].vertex_TR.uv2.x = PackUV(x2, y2);
						characterInfo[j].vertex_TR.uv2.y = num47;
						characterInfo[j].vertex_BR.uv2.x = PackUV(x2, y);
						characterInfo[j].vertex_BR.uv2.y = num47;
						break;
					}
					}
					if (j < m_maxVisibleCharacters && num42 < m_maxVisibleWords && lineNumber < m_maxVisibleLines && m_overflowMode != TextOverflowModes.Page)
					{
					...
					else if (j < m_maxVisibleCharacters && num42 < m_maxVisibleWords && lineNumber < m_maxVisibleLines && m_overflowMode == TextOverflowModes.Page && characterInfo[j].pageNumber == num10)
					{
						characterInfo[j].vertex_BL.position += zero3;
						characterInfo[j].vertex_TL.position += zero3;
						characterInfo[j].vertex_TR.position += zero3;
				...
				if (lineNumber != num43 || j == m_characterCount - 1)
				{
					if (lineNumber != num43)
					{
						m_textInfo.lineInfo[num43].baseline += zero3.y;
						m_textInfo.lineInfo[num43].ascender += zero3.y;
						m_textInfo.lineInfo[num43].descender += zero3.y;
						m_textInfo.lineInfo[num43].lineExtents.min = new Vector2(m_textInfo.characterInfo[m_textInfo.lineInfo[num43].firstCharacterIndex].bottomLeft.x, m_textInfo.lineInfo[num43].descender);
						m_textInfo.lineInfo[num43].lineExtents.max = new Vector2(m_textInfo.characterInfo[m_textInfo.lineInfo[num43].lastVisibleCharacterIndex].topRight.x, m_textInfo.lineInfo[num43].ascender);
					}
				...
				if (char.IsLetterOrDigit(character3) || character3 == '-' || character3 == '\u00ad' || character3 == '‐' || character3 == '‑')
				{
					...
					if (flag11 && j == m_characterCount - 1)
					{
						...
						num45 = j;
						m_textInfo.wordInfo[wordCount].firstCharacterIndex = num44;
						m_textInfo.wordInfo[wordCount].lastCharacterIndex = num45;
						m_textInfo.wordInfo[wordCount].characterCount = num45 - num44 + 1;
				...
				{
					num45 = ((j != m_characterCount - 1 || !char.IsLetterOrDigit(character3)) ? (j - 1) : j);
					flag11 = false;
					int num65 = m_textInfo.wordInfo.Length;
					int wordCount2 = m_textInfo.wordCount;
					if (m_textInfo.wordCount + 1 > num65)
					{
						TMP_TextInfo.Resize(ref m_textInfo.wordInfo, num65 + 1);
					}
					m_textInfo.wordInfo[wordCount2].firstCharacterIndex = num44;
					m_textInfo.wordInfo[wordCount2].lastCharacterIndex = num45;
					m_textInfo.wordInfo[wordCount2].characterCount = num45 - num44 + 1;
				...
				if ((m_textInfo.characterInfo[j].style & FontStyles.Underline) == FontStyles.Underline)
				{
					...
					{
						num50 = Mathf.Max(num50, m_textInfo.characterInfo[j].scale);
						num51 = Mathf.Min((pageNumber != num52) ? TMP_Text.k_LargePositiveFloat : num51, m_textInfo.characterInfo[j].baseLine + base.font.fontInfo.Underline * num50);
						num52 = pageNumber;
					}
					if (!flag && flag14 && j <= tMP_LineInfo.lastVisibleCharacterIndex && character3 != '\n' && character3 != '\r' && (j != tMP_LineInfo.lastVisibleCharacterIndex || !char.IsSeparator(character3)))
					{
						...
						start = new Vector3(m_textInfo.characterInfo[j].bottomLeft.x, num51, 0f);
						color = m_textInfo.characterInfo[j].underlineColor;
					}
					if (flag && m_characterCount == 1)
					{
						flag = false;
						zero = new Vector3(m_textInfo.characterInfo[j].topRight.x, num51, 0f);
						num49 = m_textInfo.characterInfo[j].scale;
						DrawUnderlineMesh(start, zero, ref index, num48, num49, num50, num47, color);
						num50 = 0f;
						num51 = TMP_Text.k_LargePositiveFloat;
					}
					else if (flag && (j == tMP_LineInfo.lastCharacterIndex || j >= tMP_LineInfo.lastVisibleCharacterIndex))
					{
						if (char.IsWhiteSpace(character3) || character3 == '\u200b')
						{
							...
							zero = new Vector3(m_textInfo.characterInfo[lastVisibleCharacterIndex].topRight.x, num51, 0f);
							num49 = m_textInfo.characterInfo[lastVisibleCharacterIndex].scale;
						}
						else
						{
							zero = new Vector3(m_textInfo.characterInfo[j].topRight.x, num51, 0f);
					...
					{
						flag = false;
						zero = new Vector3(m_textInfo.characterInfo[j - 1].topRight.x, num51, 0f);
						num49 = m_textInfo.characterInfo[j - 1].scale;
						DrawUnderlineMesh(start, zero, ref index, num48, num49, num50, num47, color);
						num50 = 0f;
						num51 = TMP_Text.k_LargePositiveFloat;
					}
				...
				if (flag15)
				{
					...
					{
						flag2 = true;
						num53 = m_textInfo.characterInfo[j].pointSize;
						num54 = m_textInfo.characterInfo[j].scale;
						start2 = new Vector3(m_textInfo.characterInfo[j].bottomLeft.x, m_textInfo.characterInfo[j].baseLine + strikethrough * num54, 0f);
					...
					{
						flag2 = false;
						zero2 = new Vector3(m_textInfo.characterInfo[j].topRight.x, m_textInfo.characterInfo[j].baseLine + strikethrough * num54, 0f);
						DrawUnderlineMesh(start2, zero2, ref index, num54, num54, num54, num47, underlineColor);
					}
					else if (flag2 && j == tMP_LineInfo.lastCharacterIndex)
					{
						if (char.IsWhiteSpace(character3) || character3 == '\u200b')
						{
							...
							zero2 = new Vector3(m_textInfo.characterInfo[lastVisibleCharacterIndex2].topRight.x, m_textInfo.characterInfo[lastVisibleCharacterIndex2].baseLine + strikethrough * num54, 0f);
						}
						else
						{
							zero2 = new Vector3(m_textInfo.characterInfo[j].topRight.x, m_textInfo.characterInfo[j].baseLine + strikethrough * num54, 0f);
					...
					{
						flag2 = false;
						zero2 = new Vector3(m_textInfo.characterInfo[j - 1].topRight.x, m_textInfo.characterInfo[j - 1].baseLine + strikethrough * num54, 0f);
						DrawUnderlineMesh(start2, zero2, ref index, num54, num54, num54, num47, underlineColor);
					}
				}
				else if (flag2)
			...
			m_textInfo.wordCount = ((num42 == 0 || m_characterCount <= 0) ? 1 : num42);
