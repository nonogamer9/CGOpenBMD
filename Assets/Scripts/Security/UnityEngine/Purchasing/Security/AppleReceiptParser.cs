...
namespace UnityEngine.Purchasing.Security
{
	public class AppleReceiptParser
	{
		...
		private AppleReceipt ParseReceipt(Asn1Node data)
		{
			...
			for (int i = 0; i < childNode.ChildNodeCount; i++)
			{
				...
				if (num2 <= 12)
				{
					...
					if (num2 == 12)
					{
						string s = Encoding.UTF8.GetString(childNode3.GetChildNode(0).Data);
						appleReceipt.receiptCreationDate = DateTime.Parse(s).ToUniversalTime();
					}
				}
				else
				{
		...
		{
			string text = Encoding.UTF8.GetString(node.GetChildNode(0).Data);
