...
namespace UnityEngine.Purchasing.Security
{
	internal class X509Cert
	{
		...
		{
			string text = new UTF8Encoding().GetString(n.Data);
			...
			if (text.Length == 13)
			{
				num2 = int.Parse(text.Substring(0, 2));
				if (num2 >= 50)
				{
					num2 += 1900;
				}
			...
			{
				num2 = int.Parse(text.Substring(0, 4));
				num += 4;
			}
			int month = int.Parse(text.Substring(num, 2));
			num += 2;
			int day = int.Parse(text.Substring(num, 2));
			num += 2;
			int hour = int.Parse(text.Substring(num, 2));
			num += 2;
			int minute = int.Parse(text.Substring(num, 2));
			num += 2;
			int second = int.Parse(text.Substring(num, 2));
			num += 2;
