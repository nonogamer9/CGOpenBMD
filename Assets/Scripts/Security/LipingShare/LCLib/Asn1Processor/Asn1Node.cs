	...
	internal class Asn1Node : IAsn1Node
	{
		...
		public bool ParseEncapsulatedData
		{
			...
			set
			{
				...
				ClearAll();
				if ((tag & 0x20) != 0 || parseEncapsulatedData)
				{
		...
		public string GetLabel(uint mask)
		{
			...
			string text2 = "";
			string text3 = "";
			text3 = (((mask & 4) != 0) ? (((mask & 8) == 0) ? string.Format("(0x{0:X6},0x{1:X4})", dataOffset, dataLength) : string.Format("(0x{0:X2},0x{1:X6},0x{2:X4})", tag, dataOffset, dataLength)) : (((mask & 8) == 0) ? string.Format("({0},{1})", dataOffset, dataLength) : string.Format("({0},{1},{2})", tag, dataOffset, dataLength)));
			switch (tag)
			{
			case 3:
				if ((mask & 1) != 0)
		...
		protected bool InternalLoadData(Stream xdata)
		{
			...
			xdata.Position = position;
			int num = b & 0x1F;
			if ((b & 0x20) != 0 || (parseEncapsulatedData && (num == 3 || num == 8 || num == 27 || num == 24 || num == 25 || num == 22 || num == 4 || num == 19 || num == 16 || num == 17 || num == 20 || num == 28 || num == 12 || num == 21 || num == 26)))
			{
