...
public static class PhotonNetwork
{
	...
	private static EventCallback OnEventCall__BackingField;
	...
	public static event EventCallback OnEventCall
	{
		add
		{
			EventCallback eventCallback = OnEventCall__BackingField;
			EventCallback eventCallback2;
			do
			{
				eventCallback2 = eventCallback;
				eventCallback = Interlocked.CompareExchange(ref OnEventCall__BackingField, (EventCallback)Delegate.Combine(eventCallback2, value), eventCallback);
			}
			while ((object)eventCallback != eventCallback2);
		}
		remove
		{
			EventCallback eventCallback = OnEventCall__BackingField;
			...
			{
				eventCallback2 = eventCallback;
				eventCallback = Interlocked.CompareExchange(ref OnEventCall__BackingField, (EventCallback)Delegate.Remove(eventCallback2, value), eventCallback);
			}
			while ((object)eventCallback != eventCallback2);
		}
	}
	...
	{
		if (OnEventCall__BackingField != null)
		{
			OnEventCall__BackingField(eventCode, content, senderId);
			return true;
		}
		return false;
	}
