...
public class GameController : Photon.MonoBehaviour
{
	...
	private static MatchTimeEnded MatchFinidhed__BackingField;
	...
	public static event MatchTimeEnded MatchFinidhed
	{
		add
		{
			MatchTimeEnded matchTimeEnded = MatchFinidhed__BackingField;
			MatchTimeEnded matchTimeEnded2;
			do
			{
				matchTimeEnded2 = matchTimeEnded;
				matchTimeEnded = Interlocked.CompareExchange(ref MatchFinidhed__BackingField, (MatchTimeEnded)Delegate.Combine(matchTimeEnded2, value), matchTimeEnded);
			}
			while ((object)matchTimeEnded != matchTimeEnded2);
		}
		remove
		{
			MatchTimeEnded matchTimeEnded = MatchFinidhed__BackingField;
			...
			{
				matchTimeEnded2 = matchTimeEnded;
				matchTimeEnded = Interlocked.CompareExchange(ref MatchFinidhed__BackingField, (MatchTimeEnded)Delegate.Remove(matchTimeEnded2, value), matchTimeEnded);
			}
			while ((object)matchTimeEnded != matchTimeEnded2);
		}
	}
	...
	private void CheckMatchEndTime()
	{
		...
		if (matchIndex == prevMatchIndex + 1)
		{
			...
			if (MatchFinidhed__BackingField != null)
			{
				MatchFinidhed__BackingField();
			}
