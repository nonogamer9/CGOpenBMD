...
public class VehicleMenu : MonoBehaviour
{
	...
	public void SpawnChaseVehicle()
	{
		if (chaseCarSpawnTime == 0f)
		{
			...
			GameObject gameObject = Object.Instantiate(chaseVehicle, spawnPoint, Quaternion.LookRotation(spawnRot, GlobalControl.worldUpDir));
	...
	public void SpawnChaseVehicleDamage()
	{
		...
		{
			chaseCarSpawnTime = 1f;
			GameObject gameObject = Object.Instantiate(chaseVehicleDamage, spawnPoint, Quaternion.LookRotation(spawnRot, GlobalControl.worldUpDir));
			gameObject.GetComponent<FollowAI>().target = newVehicle.transform;
		}
	}
}
